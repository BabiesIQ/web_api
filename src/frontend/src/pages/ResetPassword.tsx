...
export function ResetPasswordPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    if (!email) {
      toast.error(t("auth.missing_email"));
      return;
    }
    try {
      const res = await resetPassword(email, values.otp, values.password);
      if (!res.success) {
        toast.error(res.error ?? t("auth.could_not_reset"));
        return;
      }
      toast.success(t("auth.pwd_reset_success"));
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              className={`bg-background/60 border-border h-14 font-mono tracking-widest text-center text-xl transition-colors ${
                errors.otp
            ...
            {errors.otp && (
              <p
                ...
                data-ocid="reset_password.otp.field_error"
              >
